...
void createVillageHouse(int hid, int x, int y, int hw, int hh, int ex, int ey, int w, int h, int level, u8 * map, u8 * data) {
    ...
    //create npcs
    if(hid==0) {
        addEntityToList(newEntityNPC(NPC_GIRL, (x+hw/2) << 4, (y+hh/2) << 4, level), &eManager);
    } else if(hid==1) {
        addEntityToList(newEntityNPC(NPC_PRIEST, (x+hw/2) << 4, (y+hh/2) << 4, level), &eManager);
    } else if(hid==2) {
        addEntityToList(newEntityNPC(NPC_FARMER, (x+hw/2) << 4, (y+hh/2) << 4, level), &eManager);
...
void createDungeonRoom(int w, int h, bool dragon, int level, u8 * map, u8 * data) {
    ...
    for(tries=0; tries<100; ++tries) {
        ...
        if(dragon) {
            ...
            //add Dragon Entity
            addEntityToList(newEntityDragon((x+wr/2) << 4, (y+hr/2) << 4, level), &eManager);
            break;
        }
            ...
            for(yr = y; yr < y+hr; ++yr) {
                ...
                if(lava && xr > x+1 && xr < x+wr-2 && yr > y+1 && yr < y+hr-2) {
                    ...
                    if(xr==x+wr/2 && yr==y+hr/2) {
                        ...
                        if(pcount<8) {
                            addEntityToList(newEntityMagicPillar((xr << 4) + 8, (yr << 4) + 8, level), &eManager);
                        }
                        continue;
                    }
