...
void addItemsToWorld(Item item, s8 level, int x, int y, int count) {
    int i;
    for(i = 0; i < count; ++i) addEntityToList(newEntityItem(item, x, y, level), &eManager);
}
...
void hurtEntity(Entity *e, int damage, int dir, u32 hurtColor, Entity *damager) {
    ...
    addEntityToList(newParticleText(hurtText, hurtColor, e->x, e->y, e->level), &eManager);
    ...
    switch(e1->type){
        ...
        case ENTITY_SLIME:
            if(e2->type == ENTITY_PLAYER){ 
                ...
                else if(e1->type == ENTITY_SLIME) hurtEntity(e2, 1, e1->hostile.dir, 0xFFAF00FF, e1);
    ...
    switch(tile){
        ...
        case TILE_TREE:
            if(eManager.lastSlot[level]<800 && (worldData.daytime>18000 || worldData.daytime<5000) && rand()%800==0) {
                ...
                addEntityToList(newEntityGlowworm((x<<4)+8, (y<<4)+8, level), &eManager);
            }
            break;
        ...
        if (!tileIsSolid(worldData.map[level][rx+ry*128],&e)) {
            if(level==1 && (rand()%2)==0) { //passive entities on overworld
                ...
                case 0:
                    ...
                    break;
                case 1:
                    e = newEntityZombie(lvl, ex, ey, level);
                    break;
                case 2:
                    e = newEntitySkeleton(lvl, ex, ey, level);
                    break;
                case 3:
                    e = newEntityKnight(lvl, ex, ey, level);
                    break;
                }
            }
...
void playerHurtTile(PlayerData *pd, int tile, s8 level, int xt, int yt, int damage, int dir){
    ...
    switch(tile){
        case TILE_TREE:
            if(rand()%120==0)addEntityToList(newEntityItem(newItem(ITEM_APPLE,1), (xt<<4)+8,(yt<<4)+8, level), &eManager);
